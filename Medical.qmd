...
```{r load, echo = FALSE, include = FALSE, message = FALSE, warning = FALSE}
...
sym_ <- read_sheet("https://docs.google.com/spreadsheets/d/1u5jGDmdz7ROWMQYDdtfwjMpeYMcsGGKz_XZoQpA6WFc/edit#gid=241327815") %>% 
...
pain1 %>% 
  ...
  geom_point(data = sym %>% filter(Swimming == "Yes", date > (today() - 30)), 
             ...
             fill = "#0CEBFF", 
             size = 5.5) +
  scale_x_date(breaks = date_breaks(width = "2 days"),
               labels = date_format("%b %d"),
               expand = c(.01,.01)) +
  scale_y_continuous(breaks = seq(0,10,2),
                     limits = c(1,10)) +
  scale_color_manual(values = c("#7400CC", "#CC0AA4", "#0E0ACC", 
                                "#3ACC14", "#1471CC", "#805713", "#4F008C"),
                     limits = c("overall", "forearm", 
                                "back", "fatigue",
                                "leg", "dizziness", "headache"),
...
mental <- sym %>% 
  pivot_longer(c(mood, anxiety, focus, fatigue, productivity)) 
...
pain %>% 
  ...
  geom_text(data = k %>% filter(date > (today() - 140)),
            ...
            family = "Changa",
            color = "#310873") +
  geom_text(data = rb %>% filter(date > (today() - 140)),
            aes(mean(date), 11.25, 
                label = "rainbow"),
            size = 3.5,
            family = "Changa",
...
sym %>% 
  ...
  geom_curve(x = as.Date("2023-09-19"), 
             ...
             color = "#310873") +
  annotate("text", 
           x = as.Date("2023-10-03"),
           y = 1.6,
           label = "Clindamycin\n& Rifampin\nStarted",
           family = "Changa",
           color = "#310873") +
  geom_curve(x = as.Date("2023-10-04"), 
             xend = as.Date("2023-10-13"), 
             y = 2, yend = 2.25,
             curvature = -0.25,
             arrow = arrow(angle = 25,
                           length = unit(0.75, "lines")),
  ...
  annotate("text", 
           ...
           angle = 90) +
  annotate("text", 
           x = as.Date("2023-11-26"),
           y = 2.5,
           label = "Doxy\nStarted",
           family = "Changa",
           color = "#310873") +
  geom_curve(x = as.Date("2023-11-26"), 
             xend = as.Date("2023-11-20"), 
             y = 2.7, yend = 2.95,
             curvature = 0.25,
             arrow = arrow(angle = 25,
                           length = unit(0.75, "lines")),
  ...
  annotate("text", 
           ...
           family = "Changa",
           color = "#310873") +
  geom_curve(x = as.Date("2023-12-20"), 
             ...
             color = "#310873") +
  annotate("text", 
           x = as.Date("2024-01-26"),
           y = 2.5,
           label = "Doxy\nEnded",
           family = "Changa",
           color = "#310873") +
  geom_curve(x = as.Date("2024-01-26"), 
  ...
  geom_point(size = 6, 
             shape = 22) +
  scale_x_date(breaks = date_breaks(width = "3 weeks"),
...
p2 %>% 
  ...
  scale_y_date(breaks = date_breaks(width = "7 days"),
               labels = date_format("%b %d"),
               expand = c(0,0)) +
...
ggsave("story.jpg", height = 14, width = 6.5,
...
sts %>% 
  ggplot(aes(reorder(state, mean_mood), mean_mood, fill = state)) +
  geom_col() +
  scale_y_continuous(breaks = seq(0,10,2), 
                     limits = c(0,10)) +
  scale_fill_manual(values = colors) +
  labs(x = NULL,
       ...
       fill = NULL,
       title = "Happiness by State") +
  th_med +
  theme(panel.grid.major.x = element_blank())

ggsave("states.jpg", height = 6 / 1.618, width = 6,
       device = png, type = "cairo", dpi = 300)
